/* Low-storage Runge-Kutta（13，4） time stepper class implementation file.
...
// Low-storage Runge-Kutta（13，4） time step evolution routine
//
...
//          all tspan values,
//            [y(t0), y(t1), ..., y(tN)]
mat LSRK13Stepper::Evolve(vec tspan, double h, vec y) {
  ...
  if (h <= 0.0) {
    cerr << "LSRK13Stepper: Illegal h\n";
    return Y;
  }
  for (size_t tstep=0; tstep<N; tstep++) {
    if (tspan(tstep+1) < tspan(tstep)) {
      cerr << "LSRK13Stepper: Illegal tspan\n";
      return Y;
    }
  }
    ...
    for (size_t i=0; i<Nint; i++) {
      ...
      for (int j=0; j<c.size(); j++) {
        if (frhs->Evaluate(t+c(j)*hcur, K1, z) != 0) {
          std::cerr << "LSRK13Stepper::Evolve: Error in ODE RHS function\n";
          return Y;
        }
